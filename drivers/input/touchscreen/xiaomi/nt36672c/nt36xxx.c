...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	for_each_child_of_node(np, temp) {
		...
		ret = of_property_read_u32(temp, "novatek,tp-vendor",
					   &temp_val);
		if (ret) {
...
static bool nvt_cmds_panel_info(void)
{
	...
	char *match = (char *)strnstr(
		saved_command_line,
		"msm_drm.dsi_display0=", strlen(saved_command_line));
	if (match) {
...
static int32_t nvt_ts_probe(struct platform_device *pdev)
{
	...
	attrs_p = (struct attribute_group *)devm_kzalloc(
		&pdev->dev, sizeof(*attrs_p), GFP_KERNEL);
	if (!attrs_p) {
